...
| 2 | Baixa o `.deb` da URL que você passou | "Downloading Haveno from URL…" (**pode levar 30–90 min pelo Tor** — a linha fica parada; normal) |
...
> **“Offline” aqui = suporte físico**, não desligar a rede. O Haveno roda com Tor ativo; a seed vai para **papel**, nunca só no PC. Depois do backup, abra o log: `~/Persistent/qa-logs/04-haveno-backup-*.txt` ([como ler](../../automacao/docs-aluno/COMO-LER-SEUS-LOGS.md)) — use `haveno-backup.sh --qa-log`.
...
> **Verifique todo binário antes de confiar (caminho manual).** Os scripts em `automacao/homelab/` já fazem
...
> xmrig (`9AC4CEA8E66E35A5C7CDDC1B446A53638BE94409`, em xmrig.com/trilha/gpg-key). Nunca rode um binário sem
...
Para quem quiser automatizar a infraestrutura, há scripts prontos em **`automacao/homelab/`** (rodam no **home lab Debian/Ubuntu**, **não** no Tails):
...
Comece pelo **`00-verificar-requisitos.sh`** (diz se dá para nó pruned, full e/ou mineração). Cada script é **independente**, mostra o **SHA256** do que baixa (confira antes de confiar) e roda num usuário dedicado `monero`. Detalhes: `automacao/homelab/README.md`.
...
- `timedatectl` pode mostrar `System clock synchronized: no` — **normal no Tails** (sincroniza via Tor no boot, não NTP). Compare `date -u` com um relógio UTC se tiver dúvida.
...
| Criar rede própria (avançado) | https://github.com/haveno-dex/haveno/blob/master/trilha/create-mainnet.md |
...
| Docs da comunidade | https://boldsuck.github.io/haveno-reto-trilha/ |
