# Módulo 2 — Whonix Online

> **Mapa principal do hub:** [README.md#trilha-linear](../../README.md#trilha-linear) (passos **8–12**)
>
> **Primeira vez?** Não comece aqui — [README — Primeira visita?](../../README.md#primeira-visita) e passos **1–7** do Módulo 1 antes do passo **8**.

Lado **online persistente** do hub: Whonix (Gateway + Workstation via Tor) + custódia **frio↔quente**
(Tails offline assina, Whonix view-only transmite).

**Pré-requisito:** [Módulo 1](../m1-tails-haveno/README.md) passos **1–4** + **Feather no Tails** ([Vol II Cap. 3](../m1-tails-haveno/Expansao-Curso/Curso-Rede-Descentralizada-Extensao.md#3-feather-no-tails--instalar-e-verificar)) — **não** exige ter tradeado.

## Trilha deste módulo

| Passo hub | O que fazer | Livro | Comandos |
|:---------:|-------------|-------|----------|
| **8** | Mapa + escolher trilha A **ou** B | [00-Comece-aqui](00-Comece-aqui-Escolha-sua-trilha.md) | [00-Comece-aqui](00-Comece-aqui-Escolha-sua-trilha.md) |
| **9** | Confirmar 2× cópias físicas da seed | [Curso Cap. 5](Curso-Whonix-Online.md#5-cold-tails-hot-whonix) | [qa-confirm-passo9.sh](../../automacao/tails/qa-confirm-passo9.sh) |
| **10** | Instalar Whonix + PGP (detalhe por SO no Livro) | [Instalar por SO](Instalar-Whonix-passo-a-passo-por-SO.md) | [whonix-verify-image.sh](../../automacao/whonix-host/whonix-verify-image.sh) · [Playbooks M2](Playbooks/Playbooks.md) |
| **11** | Teoria cold-signing | [Curso Cap. 5](Curso-Whonix-Online.md#5-cold-tails-hot-whonix) | — |
| **12** | Cold-signing — **UMA** trilha | [Curso §5.5](Curso-Whonix-Online.md#5-cold-tails-hot-whonix) | **A:** [Trilha A](Trilha-A-Feather/Playbook-Feather-GUI.md) · **B:** [Trilha B](Trilha-B-CLI/Playbook-monero-wallet-cli.md) |

### Passo 12 — escolha UMA trilha (não misture)

| Trilha | Para quem | Comandos |
|--------|-----------|----------|
| **A — Feather (GUI)** | Maioria — interface | [Playbook-Feather-GUI.md](Trilha-A-Feather/Playbook-Feather-GUI.md) |
| **B — CLI** | Avançado — terminal | [Playbook-monero-wallet-cli.md](Trilha-B-CLI/Playbook-monero-wallet-cli.md) |

## Mapa rápido

| O quê | Onde |
|-------|------|
| Livro M2 | [Curso-Whonix-Online.md](Curso-Whonix-Online.md) |
| Índice completo (opcional) | [MANUAL-DO-CURSO.md](../../MANUAL-DO-CURSO.md) |
| Travou? | [README#travou-aqui](../../README.md#travou-aqui) |

> **Status:** v1 — auditado rodadas 15–18. PI-1: **2/5** prints em [screenshots-whonix/](imagens/screenshots-whonix/README.md) (`04`, `05`); faltam import + GPG.

---

*Módulo 2 · Privacy-OS-Hub · jun/2026*
