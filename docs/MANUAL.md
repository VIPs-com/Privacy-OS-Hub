...
> **`--qa-log` vale para todos os subcomandos do `hub.sh`** — `install`, `boot`, `backup`, `update`, `feather`. Via `hub.sh install --qa-log`, os scripts internos herdam e cada um grava seu `.txt`. Exporte tudo ao fim com `qa/export-logs.sh --usb`.
