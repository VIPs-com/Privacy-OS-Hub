...
flag, nada muda). Detalhe e por que `root` é bloqueado: **Apêndice B erro 14** no canônico.
...
| Situação | Comando |
|----------|---------|
| `.deb` completo só em `.download/` (ex.: ~266 MB) | `sync-hub-scripts.sh` + `haveno-setup.sh --qa-log` (promove sem re-baixar) |
| `.deb` + `.sig` já em `Install/`, falhou no install | `haveno-setup.sh --install-only --qa-log` |
| Ainda falha | [TRES-PASSOS — fallback atômico](../automacao/docs-aluno/TRES-PASSOS-HAVENO-TAILS.md) ou `automacao/tails/etapas/instalar-haveno/` |
